                            ...
                            <div class="media-content">
                                ...
                                <nav class="level is-mobile">
                                    <div class="level-left">
                                        <a class="level-item is-sparta" aria-label="heart" onclick="toogle_like('${post['_id']}', 'heart')">
                                            <span class="icon is-small">
                                                <i class="fa ${class_heart}" aria-hidden="true"></i>
                                            </span>&nbsp;<span class="like-num">${num2str(post['count_heart'])}</span>
                                        </a>
                                        <a class="level-item is-sparta" aria-label="star" onclick="toogle_star('${post['_id']}', 'star')">
                                            <span class="icon is-small">
                                                <i class="fa ${class_star}" aria-hidden="true"></i>
                                            </span>&nbsp;<span class="like-num">${num2str(post['count_star'])}</span>
                                        </a>
                                        <a class="level-item is-sparta" aria-label="thumbsup" onclick="toogle_thumbsup('${post['_id']}', 'thumbsup')">
                                            <span class="icon is-small">
...
function toogle_like(post_id, type) {
...
function toogle_star(post_id, type) {
...
function toogle_thumbsup(post_id, type) {
    ...
    if ($i_like.hasClass('fa-thumbs-up')) {
        $.ajax({
            ...
            success: function (response) {
                $i_like.addClass('fa-thumbs-o').removeClass('fa-thumbspup')
                $a_like.find('span.like-num').text(num2str(response['count']))
            }
        })
    } else {
        $.ajax({
            ...
            success: function (response) {
                $i_like.addClass('fa-thumbs-up').removeClass('fa-thumbs-o')
                $a_like.find('span.like-num').text(num2str(response['count']));
            }
        })
    }
